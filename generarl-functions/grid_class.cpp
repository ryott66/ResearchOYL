...
// int Grid::toFlatIndex(const vector<int>& indices) const {
...
Grid::Grid(const vector<int>& dims) : dimensions(dims) {
    if (dims.empty()) {
...
vector<int> Grid::getDimensions() const {
    return dimensions;
}